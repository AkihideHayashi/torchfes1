...
def replica_exchange(mol):
    ...
    if xi < p:
        ti = 1 / bi
        tj = 1 / bj
        mol[fes.p.mom][idx[i], :, :] *= torch.sqrt(tj / ti)
        mol[fes.p.mom][idx[j], :, :] *= torch.sqrt(ti / tj)
        swap(mol[fes.p.idt], idx[i], idx[j])
        swap(mol[fes.p.kbt], idx[i], idx[j])
